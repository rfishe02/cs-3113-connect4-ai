...
pie(c(tot[1,2],tot[2,2]), labels=c("First Utility","Random"), main="First Utility vs. Random in 300 Matches", col = c("white","gray"))

tot2 <- aggregate(round ~ win,data = random2,FUN = length)
pie(c(tot2[1,2],tot2[2,2]), labels=c("Second Utility","Random"), main="Second Utility vs. Random in 300 Matches", col = c("white","gray"))
